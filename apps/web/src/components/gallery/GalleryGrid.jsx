        ...
        {items.map((item) => (
          <Link to={`/gallery/${item.id}`} key={item.id} className="group flex h-full flex-col overflow-hidden rounded-lg border border-slate-200 bg-white transition-colors hover:border-primary/30">
            ...
            <div className="p-6 flex flex-col flex-grow">
              <h4 className="text-xl font-bold text-on-surface mb-3 leading-tight line-clamp-2">{item.title}</h4>
              <p className="text-sm text-on-surface-variant mb-6 line-clamp-2 flex-grow">{item.description}</p>
